...
class cameraFeed(Node):
    ...
    def tf_callback(self, TFMessage):
        for mes in TFMessage.transforms:
            ...
            splitUp = xyz.split(", ")
            self.equalX = "x=" + str(int((float(splitUp[0].split("=")[1]) * 100.0)))
            self.equalY = "y=" + str(int((float(splitUp[1].split("=")[1]) * 100.0)))
            self.equalZ = "z=" + str(int((float(splitUp[2].split("=")[1]) * 100.0)))
    ...
    def synchronized_callback(self, image_sub, tag_sub):
        ...
        for det in tag_sub.detections:
            ...
            cv.putText(current_frame, self.equalX, (10, 100), font, 1, (0, 255, 0), 1, cv.LINE_AA)
            cv.putText(current_frame, self.equalY, (10, 150), font, 1, (0, 255, 0), 1, cv.LINE_AA)
            cv.putText(current_frame, self.equalZ, (10, 200), font, 1, (0, 255, 0), 1, cv.LINE_AA)
                 
            
         
        
        resize = cv.resize(current_frame, (960,540))
        cv.imshow("", resize)
